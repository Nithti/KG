...
export default {
  ...
  methods: {
    ...
    initGraph() {
      ...
      // .force('x',d3.forceX().x(5000))
      // .force('y',d3.forceY().y(5000))
      // .force("center", d3.forceCenter(this.width / 2, this.height / 2));
    ...
    createMenuButton(
      ...
      menuItems.forEach((item, index) => {
        ...
        buttonEnter.on("click", function (d, i) {
          //console.log(d)
          // console.log(currentItem.childrens.length)
    ...
    drawNode(node) {
      ...
      nodeEnter.attr("r", function (d) {
        // console.log(d.r ? parseInt(d.r) : 25);
      ...
      nodeEnter.on("mouseleave", function (d) {
        ...
        // _this.showInfo=0;console.log("show"+_this.showInfo);
      });
    ...
    mergeNodeAndLink(newNodes, newLinks) {
      ...
      newNodes.forEach(function (m) {
        let sobj = _this.initData.nodes.find(function (x) {
          return x.uuid === m.uuid;
        });
        if (typeof sobj == "undefined") {
          _this.initData.nodes.push(m);
        }
      });
      newLinks.forEach(function (m) {
        let sobj = _this.initData.links.find(function (x) {
          return x.uuid === m.uuid;
        });
        if (typeof sobj == "undefined") {
