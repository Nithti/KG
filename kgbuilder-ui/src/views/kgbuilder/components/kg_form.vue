<template>
  <el-drawer ref="drawer" title="编辑" :with-header="false" :visible.sync="drawerShow" :direction="direction"
    :append-to-body="true">
    <!--导入-->
      ...
      <el-form>
        ...
        <el-form-item>
          <div v-if="uploadParam.type == 1">
            ...
            <el-carousel>
              <el-carousel-item v-for="item in [require('@/assets/sanyuanzuimport1.png')]" :key="item">
                <img :src="item" />
              </el-carousel-item>
            </el-carousel>
          </div>
          <div v-else style="max-height:calc(100vh - 80px);over-flow-y:scroll">
            ...
            <el-carousel height="450px">
              <el-carousel-item v-for="item in [require('@/assets/treeimport1.png'), require('@/assets/treeimport2.png')]"
                :key="item">
        ...
        <el-form-item label="图谱领域" label-width="120px">
          <el-input style="width:100%" v-model="uploadParam.domain" placeholder="请输入内容">
          </el-input>
        </el-form-item>
        <el-form-item label="选择文件" label-width="120px">
          <el-upload class="" ref="uploadExcel" :action="uploadGraphUrl" accept=".csv,.xls,.xlsx" :show-file-list="true"
            :data="uploadParam" :on-success="uploadExcelSuccess" :auto-upload="false">
    ...
    <div v-show="operate == 'export'" class="pd-20">
      <el-form>
        <el-form-item label="图谱领域" label-width="120px">
          <el-autocomplete style="width:100%" v-model="uploadParam.domain"
    ...
    <div v-show="operate == 'nodeEdit'" class="pd-20">
        ...
        <el-tab-pane label="属性编辑" name="propEdit">
          <el-form :model="graphData">
            ...
            <el-form-item label="选择颜色" label-width="120px">
              <el-color-picker id="colorpicker" v-model="graphData.color" :predefine="predefineColors">
              </el-color-picker>
            </el-form-item>
        ...
        <el-tab-pane label="添加图片" name="propImage">
          <el-form>
            <el-form-item label="本地上传" label-width="120px">
              <el-upload class="" name="file" ref="upload" :headers="uploadHeader" :action="uploadFileUrl"
                accept=".jpg,.png" :multiple="false" :show-file-list="false" :data="uploadImageParam"
                :before-upload="beforeUpload" :on-success="uploadSuccess" :auto-upload="true" :limit="1">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-show="propActiveName == 'propImage'" type="primary" @click="saveNodeImage"
          class="btn-line cur">保存</el-button>
        <el-button v-show="propActiveName == 'richTextEdit'" @click="saveNodeContent" type="primary"
          class="btn-line cur">保存</el-button>
        <el-button v-show="propActiveName == 'propEdit' && graphData.uuid != 0" type="primary"
          @click="createNode">更新</el-button>
        <el-button v-show="propActiveName == 'propEdit' && graphData.uuid == 0" type="primary"
          @click="createNode">创建</el-button>
    ...
    <!-- 显示预警方案 -->
    <!-- <div class="mind-cenbar">
    <el-scrollbar class="scrollbar"> -->
    <!-- <el-scrollbar class="mind-cen"></el-scrollbar> -->
    <div v-show="operate == 'yujingfangan'">
      <!-- <div class="mb-l">预警方案</div> -->
      <p class="bolder" style="font-size:20px;">{{ this.yujingnodename }}</p>
      <p class="bolder">{{ "预警级别:" }}</p>
      <p>{{ this.yujingLevel }}</p>
      <p class="bolder">特征:</p>
      <p>无数据</p>
      <p class="bolder">应急响应:</p>
      <p id="emergenceInfo">{{ this.yujingResponse ? this.yujingResponse : '无数据' }}</p>
      <p class="bolder">人员转移路线、安置点及受影响住户:</p>
      <p>无数据</p>
      <p class="bolder">抢险队伍责任人、组成及联系方式:</p>
      <p>无数据</p>
      <p class="bolder">物资保障责任人及联系方式:</p>
      <p>无数据</p>
      <p class="bolder">物资储备:</p>
      <p>无数据</p>
      <p class="bolder">三个责任人及联系方式:</p>
      <p class="bolder">政府负责人</p>
      <p>{{ this.threeperson.gov_responsible_name + ":" + this.threeperson.gov_responsible_phone }}</p>
      <p>{{ this.threeperson.gov_responsible_post }}</p>
      <p class="bolder">巡逻负责人</p>
      <p>{{ this.threeperson.patrol_responsible_name + ":" + this.threeperson.patrol_responsible_phone }}</p>
      <p>{{ this.threeperson.patrol_responsible_post }}</p>
      <p class="bolder">技术负责人</p>
      <p>{{ this.threeperson.technical_responsible_name + ":" + this.threeperson.technical_responsible_phone }}</p>
      <p>{{ this.threeperson.technical_responsible_post }}</p>
    </div>
    <!-- </el-scrollbar>
</div> -->
  </el-drawer>
...
export default {
  ...
  methods: {
    ...
    //预警方案
    getYujingfangan(drawerShow, operate, node, domain) {
      this.operate ='yujingfangan'
      this.drawerShow = drawerShow;
      this.domain = domain;
      this.batchCreateData.sourceUuid = node.uuid;
      this.propActiveName = "propEdit";
      ...
      kgBuilderApi.getMoreRelationNode(data).then(result => {
        if (result.code == 200) {
          ...
          this.yujingnodename = result.data.node[1].name
          this.yujingLevel = result.data.node[0].yj//预警等级
          this.threeperson = result.data.node[5] // 三个联系人
          //预案
          if (this.yujingLevel == "正常") {
            this.yujingResponse = "无预警"
          }
          else if (this.yujingLevel == "4") {
            this.yujingResponse = result.data.node[0].three_level_4_dispatch

          }
          else if (this.yujingLevel == "3") {
            this.yujingResponse = result.data.node[0].three_level_3_dispatch

          }
          else if (this.yujingLevel == "2") {
            this.yujingResponse = result.data.node[0].three_level_2_dispatch

          }
          else if (this.yujingLevel == "1") {
            this.yujingResponse = result.data.node[0].three_level_1_dispatch
          }
          document.getElementById('emergenceInfo').innerHTML=this.yujingResponse
          if(operate=="allLevel"){
            document.getElementById('emergenceInfo').innerHTML="<p>一级："+result.data.node[0].three_level_1_dispatch+"</p>"+"<p>二级："+result.data.node[0].three_level_2_dispatch+"</p>"+"<p>三级："+result.data.node[0].three_level_3_dispatch+"</p>"+"<p>四级："+result.data.node[0].three_level_4_dispatch+"</p>"
          }
        }
      });
    },
