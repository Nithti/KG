...
class kgBuilderApi extends BaseAPI{
  // 获取图谱数据
  getLocation(data) {
    return this.get(`/datasource/getLocation?ad_cd=${data.adcd}`);
  }
  getYjNum() {
    return this.get("/datasource/getYjNum");
  }
  getKgData() {
  ...
  getWaterLevelPrediction(data){
    return this.get('/datasource/waterlevel',data
    );
  }
  saveData(data) {
